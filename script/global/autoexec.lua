...
--	end
	...
	if tonumber(date("%y%m%d")) >= 140124 and tonumber(date("%y%m%d")) <= 540203 then
...
function AddNpcWeekEnd()
    local npcIndexZzd = CreateNpc("Tèng T­íng Qu©n", "Ch©n Tö §¬n", 200, 1380, 2872);
    SetNpcScript(npcIndexZzd, "\\script\\task\\WeekEnd\\Õç×Óµ¤.lua");

    local npcXyZzd = CreateNpc("Tèng T­íng Qu©n", "Ch©n Tö §¬n", 350, 1452, 2967);
    ...
    local npcYzZzd = CreateNpc("Tèng T­íng Qu©n", "Ch©n Tö §¬n", 150, 1714, 3137);
...
function Addxinan()
    ...
    local npcIndexxinan24 = CreateNpc("NhÞ Hoµng Tö_§¹i Lý", "§oµn Tè ThuËn", 403,1458,2842);
    ...
    local npcIndexxinan44 = CreateNpc("Xa phu T©y Nam", "Xa phu", 400,1549,2882);
    ...
    local npcIndexxinan55 = CreateNpc("Xa phu T©y Nam", "Xa phu", 400,1574,3110);
    ...
    local npcIndexxinan57 = CreateNpc("Xa phu T©y Nam", "Xa phu", 400,1465,3110);
...
function AddXiBei()
  ...
  SetNpcScript(nNpcIdx, "\\script\\Î÷±±Çø\\·ïÏè¸®\\npc\\Ò¶ÀÏ.lua");
	...
	--¸®±±³µ·ò
	local nNpcIdx = CreateNpc("Xa phu", "Xa phu", 500,1849,3061);
	SetNpcScript(nNpcIdx, "\\script\\Î÷±±Çø\\·ïÏè¸®\\npc\\·ïÏè³µ·ò.lua");

	--¶«ÃÅ³µ·ò
	local nNpcIdx = CreateNpc("Xa phu", "Xa phu", 500,1803,3220);
	SetNpcScript(nNpcIdx, "\\script\\Î÷±±Çø\\·ïÏè¸®\\npc\\·ïÏè³µ·ò.lua");

	--Î÷ÃÅ³µ·ò
	local nNpcIdx = CreateNpc("Xa phu", "Xa phu", 500,1671,3089);
	SetNpcScript(nNpcIdx, "\\script\\Î÷±±Çø\\·ïÏè¸®\\npc\\·ïÏè³µ·ò.lua");

	--ÄÏÃÅ³µ·ò
	local nNpcIdx = CreateNpc("Xa phu", "Xa phu", 500,1671,3202);
	SetNpcScript(nNpcIdx, "\\script\\Î÷±±Çø\\·ïÏè¸®\\npc\\·ïÏè³µ·ò.lua");
...
--	SetNpcScript(nNpcIdx, 
	...
	local nNpcIdx = CreateNpc("Nga Mi ®¹i s­ tû", "Chñ D­îc ®iÕm", 426,1488,3007);
	SetNpcScript(nNpcIdx, "\\script\\Mod\\ÊÎÆ·µêÀÏ°å.lua");

	local nNpcIdx = CreateNpc("Nga Mi ®¹i s­ tû", "Chñ TiÖm Tæng Hîp", 426,1657,3094);
	SetNpcScript(nNpcIdx, "\\script\\Mod\\ÊÎÆ·µêÀÏ°å.lua");

	local nNpcIdx = CreateNpc("§oµn Ngäc Nhi", "Thñ khè", 426,1512,3096);
	SetNpcScript(nNpcIdx, "\\script\\ÖÐÔ­Ò»Çø\\Ìì²¨Ñî¸®\\npc\\ÖÐÔ­´¢Îï¹ñ.lua");

	local nNpcIdx = CreateNpc("linqinghai", "Xa phu", 426,1577,3020);
	SetNpcScript(nNpcIdx, "\\script\\Î÷ÄÏÇø\\´óÀí¸®\\npc\\´óÀí³µ·ò.lua");
...
function AddTuLinhNpc()
	...
	npcIndex = CreateNpc(tb_npc_type[1], tb_npc_type[1], 100, 1416, 2965)
...
function chieuda_baoruong()	
	...
	for i = 1, getn(tbQT_BaoRuong_Pos) do
		local npcIndex = CreateNpc("muzhi_box","ChiÕu D¹ Ph¸p B¶o",tbQT_BaoRuong_Pos[i][1],tbQT_BaoRuong_Pos[i][2],tbQT_BaoRuong_Pos[i][3]);
		SetNpcScript(npcIndex,"\\script\\vng\\quyty_box\\quyty2013_box_npc.lua");
	end
end
function xichtho_baoruong()	
	...
	for i = 1, getn(tbQT_BaoRuong_Pos) do
		local npcIndex = CreateNpc("heitie_box","XÝch Thè Cæ VËt",tbQT_BaoRuong_Pos[i][1],tbQT_BaoRuong_Pos[i][2],tbQT_BaoRuong_Pos[i][3]);
		SetNpcScript(npcIndex,"\\script\\vng\\quyty_box\\quyty2013_box_npc.lua");
	end
end
function npcVanMay()
...
function Plus123Pay()
	local npcIndexZzd = CreateNpc("Sµi Phong Nhi", "Nhµ C¸i", 100, 1408, 3008);
    SetNpcScript(npcIndexZzd, "\\script\\ÖÐÔ­¶þÇø\\ÏåÑô\\npc\\ÂÌÖñÎÌ.lua");
end
