...
function main()
	...
	if DEBUG_VERSION == 0 then
		tinsert(selTab,5,"Xãa d÷ liÖu cña bæn bang/clear_relay_data");
		--tinsert(selTab,5,"§Õn thµnh kh¸c xem thö/go_to_another_city");
	end;
...
--Midyfy:ÐÞ¸ÄÎªÖ»¼ì²é¶ÓÎéÈËÊýÓëÍæ¼ÒµÈ¼¶  by yanjun 07Äê10Ô¢21ÈÕ
function check_team()
	if LIMIT_VERSION == 0 then
		return 1;
	end;
...
function check_week_callback(szKey, nKey1, nKey2, nCount)
	...
	if nCurWeek > nAttendWeek then	--Ð¢µÄÒ»ÖÜ£¬Çå³ý½ø¶È
