...
CSTAGE1 = OO_Inherit(CBASE_STAGE);	--¼Ì³Ð»ùÀµ
--=========Ï¢ÃæÊÇ×ÓÀµÄÚÈÝµÄ¶¨ÒåºÍÖØÔØ=========================================================
CSTAGE1.nStage = 1;	
CSTAGE1.tbTimerEven = {};
CSTAGE1.nMaxNpcCount = 100;
...
function CSTAGE1:OnCreateBoss()
	...
	TM_StartTimeGuage("Bãng hé vÖ",960,0,1,tbPlayer);
	TM_StartTimeGuage("Li Phñ N÷ T­íng",930,0,2,tbPlayer);
end;
--ÔÚËÄ¸öË¢¹ÖµãË¢Ëæ»úÖÖÀµµÄ¹Ö
function CSTAGE1:CreateOpposedNpc(tbArg)	
	...
	local tbPlayer = TM_GetPlayerTable();
	TM_StopTimeGuage(1);
	TM_StartTimeGuage("Bãng hé vÖ",990,0,1,tbPlayer);
end;
--BOSSÉúÐ¡º¢
function CSTAGE1:CreateBossChild(tbArg)
	...
	local tbPlayer = TM_GetPlayerTable();
	TM_StopTimeGuage(2);
	TM_StartTimeGuage("Li Phñ N÷ T­íng",960,0,2,tbPlayer);
end;
